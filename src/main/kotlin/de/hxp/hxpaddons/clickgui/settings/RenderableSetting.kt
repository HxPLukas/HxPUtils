...
abstract class RenderableSetting<T>(
    ...
    // Not just `protected val` - callers with their own layout (e.g. a custom Screen with a wider
    // column than the normal ClickGUI panel) can widen a row so a color swatch/slider drawn at its own
    // right edge actually reaches the true edge of that wider column, without needing every subclass's
    // render() to change.
