...
import de.hxp.hxpaddons.clickgui.Panel
...
import de.hxp.hxpaddons.utils.ui.animations.EaseInOutAnimation
...
class SelectorSetting(
    ...
    // Animates alpha (not brightness/HSB - controlBg is already max-brightness translucent white) so
    // hovering still smoothly fades the box in, like the old gray38.brighter() did.
    private val color: Color get() = Colors.WHITE.withAlpha(0.08f + 0.08f * hover.percent() / 100f)
    ...
    override fun render(x: Float, y: Float, mouseX: Float, mouseY: Float): Float {
        ...
        NVGRenderer.hollowRect(x + width - 20f - currentWidth, y + defaultHeight / 2f - 10f, currentWidth + 12f, 20f, 1.5f, ClickGUIModule.clickGUIColor.withAlpha(0.45f).rgba, 5f)
        ...
        NVGRenderer.rect(x + 6, y + 37f, width - 12f, options.size * 32f, Colors.controlBg.rgba, 5f)
        ...
        for (i in options.indices) {
            ...
            if (isSettingHovered(i)) NVGRenderer.hollowRect(x + 6, optionY, width - 12f, 32f, 1.5f, ClickGUIModule.clickGUIColor.withAlpha(0.45f).rgba, 4f)
        }
