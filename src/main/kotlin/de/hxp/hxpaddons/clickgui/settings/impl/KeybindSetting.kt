...
class KeybindSetting(
    ...
    /**
     * Not just [value] directly - this also invalidates [keyNameWidth]'s cache, so
     * [de.hxp.hxpaddons.clickgui.settings.ModuleButton]'s compact keybind chip (which sets a module's
     * bind straight from the module list) doesn't leave this setting's own row in the settings column
     * showing a stale key-chip width next time it's rendered there.
     */
    ...
    override fun render(x: Float, y: Float, mouseX: Float, mouseY: Float): Float {
        ...
        NVGRenderer.rect(rectX, rectY, rectWidth, rectHeight, Colors.controlBg.rgba, 5f)
        NVGRenderer.hollowRect(rectX - 1, rectY - 1, rectWidth + 2f, rectHeight + 2f, 1.5f, ClickGUIModule.clickGUIColor.withAlpha(0.45f).rgba, 4f)
