...
import de.hxp.hxpaddons.clickgui.FilterScreen
...
object ChatFilter : Module(
    name = "Chat Filter",
    description = "Hides chat messages matching user-defined patterns (regex supported).",
) {
    val filters by ListSetting<String, MutableList<String>>("Filters", mutableListOf())
    val filterScreenScale by NumberSetting("Filter Screen Size", 1.5f, 0.5f, 3.0f, 0.05f, desc = "Changes the size of the filter manager window.")

    init {
        registerSetting(ActionSetting("Manage Filters", "Opens the filter manager to add, remove and search filters.") {
            schedule(0) { mc.setScreen(FilterScreen) }
        })

        on<ChatPacketEvent> {
            if (!enabled) return@on
            with(ChatManager) {
                if (filters.any { pattern ->
                    runCatching { Regex(pattern, RegexOption.IGNORE_CASE).containsMatchIn(value) }
                        .getOrElse { value.contains(pattern, ignoreCase = true) }
                }) hideMessage()
            }
        }
    }

    val NOAMM_PRESETS = listOf(
