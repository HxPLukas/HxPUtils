...
import de.hxp.hxpaddons.clickgui.settings.DevModule
import de.hxp.hxpaddons.clickgui.settings.Setting
...
abstract class Module(
    ...
    protected inline val mc get() = HxPMod.mc
    ...
    /**
     * Indicates if the module has the annotation [WipModule].
     *
     * Unlike [isDevModule], the module still registers and works normally (settings, keybinds, event
     * subscriptions) - it's only hidden from the ClickGUI's panels unless [ModuleManager.showWipModules]
     * is toggled on, e.g. via the `/hxp wip` command.
     */
    @Transient
    val isWip = this::class.hasAnnotation<WipModule>()
    ...
    private companion object {
        private fun getCategoryFromPackage(clazz: Class<out Module>): Category {
            ...
            return when {
                ...
                packageName.contains("garden") -> Category.GARDEN
                ...
                packageName.contains("render") -> Category.RENDER
                    ...
                    "Module ${clazz.name} failed to get category from the package it is in." +
                            ...
                            " or put it under any valid package (general, garden, skyblock, render))"
                )
            }
        }
    }
