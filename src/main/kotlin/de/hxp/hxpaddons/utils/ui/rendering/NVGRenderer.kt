...
import de.hxp.hxpaddons.utils.Color.Companion.alpha
import de.hxp.hxpaddons.utils.Color.Companion.blue
import de.hxp.hxpaddons.utils.Color.Companion.brighter
import de.hxp.hxpaddons.utils.Color.Companion.green
import de.hxp.hxpaddons.utils.Color.Companion.red
...
object NVGRenderer {
    ...
    val defaultFont = Font("Default", mc.resourceManager.getResource(Identifier.parse("hxpaddons:font.ttf")).get().open())
    ...
    /**
     * The single on/off switch look shared by every toggle in the Click GUI (module rows,
     * [de.hxp.hxpaddons.clickgui.settings.impl.BooleanSetting], [de.hxp.hxpaddons.clickgui.settings.impl.HUDSetting]
     * alike), so they can't drift apart into different styles again. A translucent "glass" track instead
     * of an opaque gray-filled bar, tinted by [accent] once on, with the knob itself recoloring to match -
     * only its slide position is animated via [anim] (read only, never started/reset here). [scale]
     * multiplies every dimension uniformly (all the base numbers below are the size at `scale = 1f`) -
     * [de.hxp.hxpaddons.clickgui.settings.ModuleButton] is the only caller that varies it today.
     */
    fun toggleSwitch(x: Float, y: Float, on: Boolean, hovered: Boolean, accent: Color, anim: LinearAnimation<Float>, scale: Float = 1f) {
        val w = 34f * scale
        val h = 20f * scale
        val r = h / 2f
        val knobR = 6f * scale
        val border = 1.3f * scale

        val trackBg = if (on) accent.withAlpha(if (hovered) 0.32f else 0.22f) else (if (hovered) Colors.controlBgHover else Colors.controlBg)
        val borderColor = if (on) accent.withAlpha(0.85f) else Colors.WHITE.withAlpha(0.18f)
        val knobColor = if (on) accent.brighter(1.15f) else Colors.WHITE.withAlpha(0.85f)

        rect(x, y, w, h, trackBg.rgba, r)
        hollowRect(x, y, w, h, border, borderColor.rgba, r)
        circle(x + 40f * scale - anim.get(30f * scale, 14f * scale, !on), y + h / 2f, knobR, knobColor.rgba)
    }
