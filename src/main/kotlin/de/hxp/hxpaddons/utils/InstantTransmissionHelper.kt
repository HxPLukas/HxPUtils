...
/**
 * Ported from NoammAddons' `InstantTransmissionHelper` (utils/items/InstantTransmissionHelper.kt): a fixed-step
 * ray march that finds the landing spot for Aspect of the Void/End's Instant Transmission (and Wither Impact,
 * which reuses the same trajectory), used by [de.hxp.hxpaddons.features.impl.skyblock.NoRotate].
 */
object InstantTransmissionHelper {
    ...
    fun predictTeleport(distance: Double, startPos: Vec3, yaw: Float, pitch: Float, sneaking: Boolean): Vec3? {
        val eyeHeight = EYE_HEIGHT - if (sneaking) SNEAK_OFFSET else .0
        var currentPosition = Vec3(startPos.x, startPos.y + eyeHeight, startPos.z)

        val direction = getLookVec(yaw, pitch)
