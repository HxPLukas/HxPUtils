...
/**
 * Draws the whole of [id] (a texture whose actual pixel size is [textureWidth]x[textureHeight]),
 * scaled to [width]x[height] on screen at [x]/[y].
 */
fun GuiGraphicsExtractor.drawImage(id: Identifier, x: Int, y: Int, width: Int, height: Int, textureWidth: Int, textureHeight: Int) {
    blit(RenderPipelines.GUI_TEXTURED, id, x, y, 0f, 0f, width, height, textureWidth, textureHeight, textureWidth, textureHeight, -1)
}

fun GuiGraphicsExtractor.text(text: String, x: Int, y: Int, color: Color = Colors.WHITE, shadow: Boolean = true) {
    this.text(mc.font, text, x, y, color.rgba, shadow)
}
