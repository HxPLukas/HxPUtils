...
import de.hxp.hxpaddons.mixin.accessors.GuiGraphicsExtractorAccessor
...
class ItemStateRenderer(vertexConsumers: MultiBufferSource.BufferSource)
    ...
    override fun renderToTexture(state: State, poseStack: PoseStack) {
        ...
        state.state.itemStackRenderState().submit(poseStack, dispatcher.submitNodeStorage, 0xF000F0, OverlayTexture.NO_OVERLAY, 0)
    ...
    companion object {
        fun GuiGraphicsExtractor.drawItemStack(item: ItemStack, x: Int, y: Int) {
            ...
            val accessor = this as GuiGraphicsExtractorAccessor
            val scissorStack = GuiGraphicsExtractor::class.java.getDeclaredField("scissorStack").also { it.isAccessible = true }.get(this)
            val scissor = scissorStack?.javaClass?.getMethod("peek")?.invoke(scissorStack) as? ScreenRectangle
