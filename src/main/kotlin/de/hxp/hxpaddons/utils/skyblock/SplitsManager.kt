package de.hxp.hxpaddons.utils.skyblock
...
import de.hxp.hxpaddons.utils.PersonalBest
            ...
            if (value == "Starting in 1 second.") {
                ...
                currentSplits = when (LocationUtils.currentArea) {
                    Island.Dungeon -> {
                        ...
                        with(dungeonSplits[floor.floorNumber].toMutableList()) {
                            ...
                            SplitsGroup(map { it.copy(time = 0L, ticks = 0L) }, null)
                        }
                    }

                    Island.Kuudra -> {
                        when (KuudraUtils.kuudraTier) {
                            5 -> SplitsGroup(kuudraT5SplitsGroup.map { it.copy(time = 0L, ticks = 0L) }, null)
                            4 -> SplitsGroup(kuudraSplitsGroup.map { it.copy(time = 0L, ticks = 0L) }, null)
                            3 -> SplitsGroup(kuudraSplitsGroup.map { it.copy(time = 0L, ticks = 0L) }, null)
                            2 -> SplitsGroup(kuudraSplitsGroup.map { it.copy(time = 0L, ticks = 0L) }, null)
                            1 -> SplitsGroup(kuudraSplitsGroup.map { it.copy(time = 0L, ticks = 0L) }, null)
                ...
                if (index == currentSplits.splits.size - 1) {
                    ...
                    schedule(10) {
                        ...
                        capturedPB?.time(capturedSplits[index - 1].name, currentSplitTime, "s§7!", "§6${capturedSplits[index - 1].name} §7took §6", false)
                        capturedPB?.time(capturedSplits[index].name, times.last() / 1000f, "s§7!", "§6Total time §7took §6", false)
                        times.forEachIndexed { i, it ->
                            val name = if (i == capturedSplits.size - 1) "Total" else capturedSplits[i].name
                            modMessage("§6$name §7took §6${formatTime((it))}§7.")
                        }
                    }
                } else currentSplits.personalBest?.time(currentSplits.splits[index - 1].name, currentSplitTime, "s§7!", "§6${currentSplits.splits[index - 1].name} §7took §6", false)
            }
        }
