...
import de.hxp.hxpaddons.utils.rotateAroundNorth
import de.hxp.hxpaddons.utils.rotateToNorth
...
object DungeonUtils {
    ...
    inline val doors
        get() = DoorScanUtils.doors

    inline val nearbyRooms
        get() = NearbyRoomScanner.rooms

    /**
     * Converts a continuous world x/z into map-space, where one 32-block room grid cell
     * (see [ScanUtils]) is exactly 20 map units wide, corner-aligned to grid cell boundaries.
     */
    fun worldToMapSpace(x: Double, z: Double): Pair<Float, Float> =
        ((x + 201.0) / (32.0 / 20.0)).toFloat() to ((z + 201.0) / (32.0 / 20.0)).toFloat()
    ...
    /**
     * Finds the already-scanned [Room] (current, passed or nearby) whose grid cell contains [pos].
     * Returns `null` for rooms that haven't been scanned yet (e.g. not yet visible/loaded), since there's
     * no live-scan fallback here - only known map data is checked.
     */
    fun roomAt(pos: Vec3): Room? {
        val roomCenter = ScanUtils.getRoomCenter(pos.x.toInt(), pos.z.toInt())
        return (passedRooms.asSequence() + nearbyRooms.asSequence() + sequenceOf(currentRoom))
            .filterNotNull()
            .find { room -> room.roomComponents.any { it.vec2 == roomCenter } }
    }
}