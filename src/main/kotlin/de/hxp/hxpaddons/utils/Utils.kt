...
fun logError(throwable: Throwable, context: Any) {
    val message =
        "${HxPMod.version} Caught an ${throwable::class.simpleName ?: "error"} at ${context::class.simpleName}."
    HxPMod.logger.error(message, throwable)

    val clipboardText = "$message\n```${throwable.message}\n${
        throwable.stackTraceToString().lineSequence().take(10).joinToString("\n")
    }```"

    modMessage(Component.literal("$message §cClick this message to copy the error to your clipboard!").withStyle {
        it
            .withClickEvent(ClickEvent.CopyToClipboard(clipboardText))
...
fun setClipboardContent(string: String) {
    try {
        ...
        HxPMod.logger.error("Failed to set Clipboard Content", e)
    }
}
