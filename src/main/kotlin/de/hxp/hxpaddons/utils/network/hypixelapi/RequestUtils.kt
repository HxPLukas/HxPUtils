...
import de.hxp.hxpaddons.features.impl.render.ClickGUIModule.hypixelApiUrl
import de.hxp.hxpaddons.utils.network.WebUtils.fetchJson
...
object RequestUtils {
    ...
    /** Public, keyless Hypixel endpoint (like [de.hxp.hxpaddons.utils.network.WebUtils.hasBonusPaulScore]'s election lookup) - no uuid/auth needed. */
    suspend fun getBazaar(): Result<BazaarApiData.Reply> = fetchJson<BazaarApiData.Reply>(hypixelApiUrl + "skyblock/bazaar")

    /** Public, keyless Hypixel endpoint - the full Skyblock item list (id/name/etc, ~5MB). No recipe data (see [de.hxp.hxpaddons.utils.skyblock.recipe.RecipeRepo]). */
    suspend fun getItems(): Result<ItemsApiData.Reply> = fetchJson<ItemsApiData.Reply>(hypixelApiUrl + "resources/skyblock/items")
